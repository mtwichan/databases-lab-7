...
router.post("/", function (req, res) {
  ...
  ;(async () => {
    ...
    if (authenticatedUser.userid) {
      req.session.username = authenticatedUser["userid"]
...
async function validateLogin(req) {
  ...
  let loginAuthQuery = "SELECT * FROM customer WHERE userid = @userid AND password = @password"
  ...
  let authenticatedUser = await (async function () {
    try {
      ...
      loginAuthResult = await pool.request().input("userId", sql.NText, username).input("password", sql.NText, password).query(loginAuthQuery)
