...
const writePage = async (req, res) => {
	...
	let totalSalesQuery =
		"SELECT CAST(orderDate AS DATE) AS orderDate, SUM(totalAmount) AS totalAmount FROM ordersummary o GROUP BY CAST(orderDate AS DATE)";
	...
	try {
		...
		productResult = res.render("administrator", {
